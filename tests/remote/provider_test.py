...
async def test_full_code_exchange_roundtrip(provider):
    ...
    assert (
        provider.store.get_by_access(token.access_token)["google_refresh"]
        == "gr"
    )
...
async def test_refresh_rotates_and_preserves_google_tokens(provider):
    ...
    rt = RefreshToken(
        token="rt", client_id="c1", scopes=["openid"], expires_at=None
    )
    ...
    assert (
        provider.store.get_by_access(new.access_token)["google_refresh"] == "gr"
    )
